...
In SecretSpec 0.20+, reads accept both AWS `SecretString` and `SecretBinary`.
Writes use `SecretString` for UTF-8 values and `SecretBinary` for other bytes.
Declare a binary secret with `as_path = true`; existing inline and environment
surfaces remain UTF-8 only.
...
in SecretSpec 0.20+. References are **read-only** in this provider.
