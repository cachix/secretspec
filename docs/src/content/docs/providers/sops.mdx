...
Explicit provider credentials preserve non-UTF-8 bytes in the SOPS subprocess
environment on Unix. NUL bytes are rejected before SOPS starts; other
platforms require UTF-8 credentials.
...
- `path/to/secret` — optional absolute or relative path to the encrypted file;
  ...
  containing `secretspec.toml`, not from the shell's current directory.
...
SecretSpec asks SOPS to emit JSON when decrypting and selects the secret from
that JSON representation. When `?format=` is present, SecretSpec passes the
...
rejects refs: without convention `project`/`profile` inputs, SecretSpec cannot
