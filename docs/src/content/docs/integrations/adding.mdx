...
| [IPC](#resolve-over-ipc) | A separate program needs on-demand resolution, prompting, or write-back, and must not link SecretSpec | A protocol client | 0.21+ |
| [An in-tree integration](#ship-an-integration-inside-secretspec) | The tool speaks its own credential or helper protocol, and the integration should ship with SecretSpec | A module and a shim binary | 0.21+ |
...
The consumer launches [`secretspec serve`](/reference/cli/#serve-021) as a
...
Rust clients can also reach a resolver through SSH (0.21+), and async Rust
clients can connect an existing authenticated stream (0.21+). See
...
| --- | --- | --- |
| `secretspec-ipc` | Rust | Async by default; `blocking` supports callers without a runtime. Both support SSH (0.21+); async also accepts authenticated streams (0.21+) |
