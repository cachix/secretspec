...
| [file](/providers/file/) (0.19+) | One plaintext byte file per secret (arbitrary bytes in 0.20+) | ✓ | ✓ | ✗ | — |
...
| [systemd-credential](/providers/systemd-credential/) (0.17+) | Credentials passed to the current systemd service (arbitrary bytes in 0.20+) | ✓ | ✗ | Depends on the unit's credential source | [Via systemd-creds](https://www.freedesktop.org/software/systemd/man/latest/systemd-creds.html) |
...
| [awssm](/providers/awssm/) | AWS Secrets Manager, including `SecretBinary` (0.20+; requires the `awssm` build feature) | ✓ | ✓ | ✓ | — |
