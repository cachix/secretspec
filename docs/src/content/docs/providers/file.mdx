...
provider reads and writes one plaintext file per secret.
...
In SecretSpec 0.20+, values are read and written as arbitrary bytes. NULs,
non-UTF-8 bytes, leading and trailing whitespace, and line endings are all
preserved. Declare `as_path = true` (0.20+) to materialize binary data for an
application. Inline values, environment export, composition, and extraction
still require valid UTF-8.
...
The file provider returns the complete document; SecretSpec requires UTF-8 and then applies
