...
PHP, C#, Swift (0.18+), JVM languages (0.20+), and Elixir (0.21+). This page is for
...
This embedded boundary remains supported. SecretSpec 0.21+'s
...
[Implementing SecretSpec IPC](/development/ipc-implementation) for the 0.21+
...
| Elixir (0.21+) | `secretspec` on Hex.pm + Rustler NIF release assets | `elixir-package.yml` |

<VersionCompatibility version="0.21" />

The Elixir SDK is scheduled for SecretSpec 0.21. Its package and workflow are
available for integration testing while that release is prepared.
...
If the SDK offers resolver mode (SecretSpec 0.21+), keep it an explicit backend choice and run the
