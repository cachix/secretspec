...
- update /Cargo.toml: bump `workspace.package` version and the `secretspec` / `secretspec-core` / `secretspec-derive` / `secretspec-ipc` entries in `workspace.dependencies`; also bump the `secretspec` dependency pin in secretspec-derive/Cargo.toml (all member crates inherit `version.workspace = true`)
...
- if a publishable crate is new on crates.io (such as `secretspec-ipc` for 0.21.0), publish it by hand first following the crates.io section of `RELEASE.md`
