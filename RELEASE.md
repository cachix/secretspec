...
## Native library rename gate for 0.20

SecretSpec 0.20 renames the embedded native library from `secretspec-ffi` /
`secretspec_ffi` to `libsecretspec` without renaming its three exported C
symbols. Before tagging 0.20:
...
2. Verify Go purego, .NET, and PHP FFI discover the pre-0.20 shared-library
   filenames, and Ruby source builds discover `libsecretspec_ffi.a`.
