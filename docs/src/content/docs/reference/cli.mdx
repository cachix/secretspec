...
$ secretspec set <NAME> --from-file <FILE|-> # 0.20+
```
...
- `--from-file <FILE>` - Read the exact value bytes from `FILE`; use `-` for
  stdin (0.20+). This conflicts with the positional `VALUE`.

**Example:**
```bash
$ secretspec set API_KEY sk-1234567890 --profile production --provider sops://secrets.enc.yaml

$ secretspec set TLS_KEYSTORE --from-file client.p12 # 0.20+

$ generate-key | secretspec set TLS_KEYSTORE --from-file - # 0.20+
# SecretSpec 0.19+:
...
remain text. A binary declaration normally uses `as_path = true`; add a
