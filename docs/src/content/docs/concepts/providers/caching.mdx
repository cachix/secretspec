...
Values that are not UTF-8 are cached in a new v4 envelope that stores the
bytes as strict padded Base64. Text values keep the v3 envelope that 0.18 to
0.20 wrote, so a cache store shared with an earlier release stays usable, and
existing v2 and v3 entries remain readable.
...
reference, or `max_age` invalidates it. A UTF-8 value is written as a v3
entry whose `value` field holds the text. In SecretSpec 0.21+, any other value
is written as a v4 entry whose `value_base64` field is the cache envelope's
byte serialization; it is separate from a declaration's manifest `encoding`
and preserves the cached provider representation exactly. SecretSpec reads
v2, v3, and v4 entries.
