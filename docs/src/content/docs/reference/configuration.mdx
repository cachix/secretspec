...
composed secret materializes the final combined value. In SecretSpec 0.20+,
...
In SecretSpec 0.20+, the file always contains the exact logical bytes. When
...
the selected logical value. Binary declarations should use `as_path`; inline
SDK values and environment exports require UTF-8.
...
values can be stored through text-only providers. Decoding is strict ASCII and
does not trim line endings or whitespace.

</VersionCompatibility>

`encoding` (0.19+) defines the textual representation stored by providers. It
is independent of the cache envelope and of `as_path`: decoded UTF-8 remains
an ordinary environment or SDK value, while arbitrary decoded bytes can be
materialized to a file.
...
In SecretSpec 0.20+, the stored representation must be ASCII and contain only
the selected encoding's alphabet; whitespace, including a final LF or CRLF,
is rejected. Without `as_path = true`, decoded bytes must be valid UTF-8.

`Secrets::set` and `secretspec set --from-file` (0.20+) provide logical bytes;
positional CLI values, interactive prompts, defaults, generated secrets, and
composed results provide UTF-8 logical bytes. SecretSpec encodes logical bytes
...
In 0.20+, extraction also rejects non-UTF-8 decoded documents before parsing.
...
| [file (0.19+)](/providers/file/#use-existing-files) | Relative file path beneath the configured root | Rejected | Reads the complete file as arbitrary bytes (0.20+) | ✅ |
...
| [systemd credentials (0.17+)](/providers/systemd-credential/#use-an-existing-credential-name) | Credential filename | Rejected | Reads arbitrary credential bytes (0.20+) | — (read-only) |
...
| [AWS Secrets Manager](/providers/awssm/#use-existing-secrets) | Secret name or ARN | JSON key (UTF-8 only) | Whole `SecretString` or `SecretBinary` value (0.20+) | — (read-only) |
