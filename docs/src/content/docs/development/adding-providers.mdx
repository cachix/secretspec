...
Provider values are now arbitrary bytes held in `SecretBytes`. Text-only
providers must validate UTF-8 explicitly.
...
pub trait Provider: Send + Sync {
    ...
    /// SecretSpec 0.20+: optional expiry metadata. The default wraps `get`.
    fn get_with_metadata(&self, addr: Address<'_>)
        -> Result<Option<ProviderValue>> { /* default */ }
    ...
    fn get_many(&self, requests: &[(&str, Address<'_>)])
        -> Result<HashMap<String, SecretBytes>> { /* default */ }

    /// SecretSpec 0.20+: optional batch values with expiry metadata.
    fn get_many_with_metadata(&self, requests: &[(&str, Address<'_>)])
        -> Result<HashMap<String, ProviderValue>> { /* default */ }
...
In SecretSpec 0.20+, construct returned text with `bytes_from_text` or
`SecretBytes::from_utf8`. A provider with a byte-native API should use
