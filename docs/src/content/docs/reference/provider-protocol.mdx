...
available starting with SecretSpec 0.21. This specification replaces the
...
side-effect-free `rpc.discover` method (0.21+) without initializing the provider.
...
- `context` (0.21+) carries the resolver-declared project, active profile, absolute
...
reference (0.21+). Its `id` correlates the failed operation with a
...
The terminal prompt described below is specific to the SecretSpec CLI's
credential handler.

During `secretspec set` (0.21+), the same callback can request a
...
credential. SecretSpec first looks for an existing value, then prompts with
...
`expires_at_unix_ms` (0.21+) is required on a found result and may be null. A
