...
# libsecretspec.a) and place it on the SDK's load path for dev and tests.
...
# from libsecretspec.pc (via PKG_CONFIG_PATH) instead.
set -euo pipefail
...
if [ -z "${SECRETSPEC_FFI_STATICLIB:-}" ] && [[ " $* " != *" --enable-pkg-config "* ]]; then
  cargo build -p libsecretspec --manifest-path "$repo_root/Cargo.toml"
fi
