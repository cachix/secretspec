...
are available starting with SecretSpec 0.21.
...
newline-delimited JSON frames. Optional Rust resolver connections (0.21+) carry
...
- **No bidirectional calls.** Varlink streams replies from a service, but does
  ...
  `client.credential` (0.21+); a second connection would add discovery,
...
adopting its wire protocol: `rpc.discover` (0.21+) returns a self-contained
...
1. The wire layer owns frames, JSON-RPC envelopes, side-effect-free discovery
   (0.21+), initialization, request IDs, deadlines, cancellation, shutdown,
...
The launcher owns environment inheritance policy. SecretSpec starts external
...
Optional Rust resolver connections (0.21+) separate session ownership from child
...
provider-reported secret expiry on reads starting with SecretSpec 0.21. It does
