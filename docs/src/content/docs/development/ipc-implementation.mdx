...
with SecretSpec 0.21. Release validation still requires the pure-C client, Rust
...
Protocol version 1 becomes immutable when SecretSpec 0.21 is released. Before
...
After 0.21, a breaking correction uses a new protocol integer. A bug fix may
...
secretspec-ipc/                  # Full Rust client/server implementation
  schema/ipc/v1/                 # Packaged discovery descriptions (0.21+)
  src/
    description.rs
    frame.rs
    client.rs
    connection.rs               # SSH options and filesystem policy (0.21+)
    jsonrpc.rs
    lifecycle.rs
    server.rs
    resolver.rs
...
pub struct RequestContext {
    ...
    /// Calls back to the client on this session (0.21+). Bounded by this
...
Starting with 0.21, the claim intentionally declares only executable identity. Credential names
...
stream (0.21+); process cleanup belongs only to the launched variant. Both Rust
resolver clients support SSH launch (0.21+), using the common `SshOptions` and
...
server additionally answers side-effect-free `rpc.discover` (0.21+). Runtime
...
- side-effect-free `rpc.discover` (0.21+) before and after initialization,
...
The SecretSpec 0.21+ repository also links the pure-C client into the native
...
`Revision` is a bounded opaque token in `secretspec-ipc`. Provider values carry
it alongside their bytes and expiry; both `get_with_metadata` and
`get_many_with_metadata` must preserve it. SecretSpec caches this provider token
...
| Domain | Fields |
| --- | --- |
| `secretspec.awssm.v1` | Returned full ARN, returned version ID |
| `secretspec.awssm.selection.v1` | Previous token, `whole` or `field`, field name (empty for whole-object reads) |
| `secretspec.logical.v1` | Provider token, extraction format or empty, extraction pointer or empty, encoding name or empty |
