...
- store or remove one exact declared secret name (0.21+), when the endpoint
...
--read-only` (0.21+), which advertises resolution only.
...
The optional SSH connector (0.21+) directly launches the local SSH executable.
...
(0.21+), which does not initialize the resolver. A successful initialization
...
[`client.prompt` callback](#ask-the-client-for-a-value) (0.21+).
...
### Remote resolver connections (0.21+)
...
```sh
# Rust SSH resolver client, SecretSpec 0.21+ on both machines
cargo run -p secretspec-ipc --example ssh_resolver -- \
  developer /home/alice/project/secretspec.toml TOKEN
```
...
### Remote filesystem and authority (0.21+)
...
SSH grants access as the remote OS account. A regular `secretspec serve` process
...
They MAY additionally include the mutation capabilities (0.21+):
...
Both absolute timestamps are available starting with SecretSpec 0.21. They are
...
backend is unchanged. A source without SecretSpec caching is needed when each
...
fingerprint one resolution and launch another resolution through `secretspec run`.
...
`client.prompt` is available starting with SecretSpec 0.21.
...
- `libsecretspec-resolver` answers prompts without a callback (0.21+). Its ABI
...
0.21, and only from endpoints that advertise them.
...
For remote Rust connections (0.21+), call `connect` on a newly authenticated
