...
import promptMissingExample from '../../../../../secretspec-derive/examples/prompt_missing.rs?raw';
...
## Listing providers {/* #listing-providers-022 */}

<VersionCompatibility version="0.22" />

Use `providers()` to list built-in provider names, descriptions, and example
URIs without loading a manifest or connecting to a provider:
...
The list also includes providers disabled by Cargo features. Attempting to use
one of those providers reports the feature required to enable it. External
providers discovered at runtime are not in this list.
...
In 0.21+, `validate()` retains inline `SecretBytes` in `resolved.secrets`.
...
with `SecretSpecError::SecretNotText`, which names the secret without exposing
its bytes (0.21+). Composition and extraction also validate UTF-8, returning
...
environment, `try_as_env_value()` or `try_as_env_value_for(name)` (0.21+)
