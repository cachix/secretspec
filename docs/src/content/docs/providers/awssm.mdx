...
In SecretSpec 0.21+, reads accept both AWS `SecretString` and `SecretBinary`.
Writes use `SecretString` for UTF-8 values and `SecretBinary` for other bytes.
In 0.21+, `get` and the Rust byte resolution APIs return binary values inline,
...
in SecretSpec 0.21+. References are **read-only** in this provider.
...
SecretSpec also accounts for its own extraction and decoding before returning
...
Both single and batch reads preserve revisions. A SecretSpec cache returns the
