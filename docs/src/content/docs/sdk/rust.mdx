...
## Setting byte values {/* #setting-byte-values-020 */}

<VersionCompatibility version="0.20" />

`SecretBytes` is the zeroizing byte container used by provider and write APIs.
...
allows binary values to use a text-only provider. Declare binary values with
`as_path = true`; generated `String` fields, inline JSON responses,
composition, extraction, and environment export reject non-UTF-8 values with
an error.
