...
SecretSpec refuses symlinks, directories, and credential names that could
escape the credential directory. In SecretSpec 0.20+, it reads each credential
byte-for-byte. Inline consumers still require UTF-8, while `as_path = true`
materializes arbitrary credentials exactly.
