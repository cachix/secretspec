...
tooling, and earlier SecretSpec releases keep reading it. A value with line
breaks, leading or trailing whitespace, NUL bytes, or non-UTF-8 content would
not survive that path, so SecretSpec stores it with `gopass cat` in gopass's
...
Text entries, whether written by SecretSpec or created with `gopass insert`,
are read from their password line only: SecretSpec returns the first line with
surrounding whitespace removed, so a multiline value stored that way comes back
truncated with no error. Writing the secret again with `secretspec set` stores
a multiline value in the binary-entry format, which preserves every byte.
Reading a binary entry costs two decryptions, because SecretSpec asks for the
