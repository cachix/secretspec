...
| Access | KDBX 3 read; KDBX 4.1 read and write; KDBX 4.0 writes supported in 0.21+ |
...
<VersionCompatibility version="0.21" kind="changed">
  Writing a KDBX 4.0 database, including one saved by KeePassXC, upgrades its
  file format to KDBX 4.1. Encryption and key-derivation settings, including
  Argon2 memory, iterations, and parallelism, are preserved. Earlier versions
  could read these files but failed to write them with “Unsupported database
  version”.
</VersionCompatibility>

Reads open KDBX 3 and KDBX 4 databases. Writes create KDBX 4.1 databases and
