...
- **Elixir (0.21+)** uses a pure Elixir client for `secretspec.resolver/1` and launches `secretspec serve` directly.
...
SecretSpec 0.21+, provider and cache internals preserve arbitrary bytes, but
the existing JSON and typed `String` SDK surfaces still validate UTF-8. They do
not silently return Base64 for binary values.
Rust callers can use `resolve_bytes()` and `resolve_named_bytes()` (0.21+) to
receive inline `SecretBytes`; see [Rust byte APIs](/sdk/rust/#setting-byte-values-021).
...
- **Elixir (0.21+)** is architecture-independent and requires a compatible `secretspec` executable.
