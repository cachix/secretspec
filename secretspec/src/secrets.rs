...
impl Secrets {
    ...
    ///
    /// The typed loaders generated by `secretspec-derive` call this so an
    /// environment scope cannot silently narrow a typed load below the full
    /// generated struct shape it expects. An explicitly set scope
    /// ([`Self::set_scope`]) is still honored — only the environment fallback is
    /// cut. The untyped CLI/SDK resolution paths never call this, so they keep
    /// honoring `SECRETSPEC_SCOPE`.
    ...
    fn insert_resolved(
        ...
        temp_files: &mut Vec<(String, tempfile::NamedTempFile)>,
        name: String,
        value: SecretString,
        as_path: bool,
    ) -> Result<()> {
        if as_path {
            ...
            temp_files.push((name.clone(), temp_file));
    ...
    fn scope_excluded_names(&self, profile_name: &str) -> Result<Vec<String>> {
        if self.active_scope_members()?.is_none() {
            return Ok(Vec::new());
        }
        // The visible set: the scope ∩ the selected profile — exactly what the
        // child is allowed to see. Its own composed-secret dependencies are
        // resolved but not exposed, so they are excluded here too.
        let visible: HashSet<String> = self
            .resolve_profile_secret_names(Some(profile_name))?
            .into_iter()
            .collect();
        // Scrub *every* manifest-declared secret outside the visible set, across
        // all profiles — not only the selected profile's excluded names. A secret
        // declared under another profile can reach the child through the inherited
        // parent environment (a devenv shell, a prior `eval "$(secretspec
        // export)"`); filtering only the selected profile would let it leak.
    ...
    fn execute_plan(
        ...
        // Keyed by secret name so an out-of-scope `as_path` dependency filtered
        // out of the output below also drops its backing temp file (rather than
        // leaking a persisted file on disk via `keep_temp_files`).
        let mut temp_files: Vec<(String, tempfile::NamedTempFile)> = Vec::new();
