...
responses, composition, extraction, and text exports validate UTF-8 at their
text boundaries. For a text value, use `SecretBytes::try_as_utf8()`; for a
process environment, `try_as_env_value()` (0.21+) preserves non-UTF-8 bytes on
Unix and rejects NUL bytes on every platform.
