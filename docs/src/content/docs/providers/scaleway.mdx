...
Whole secret values preserve arbitrary bytes on reads and writes. SecretSpec
handles the API's base64 representation automatically; manifest `encoding`
is not needed for binary storage. Selecting a JSON `field` still requires a
UTF-8 document.
