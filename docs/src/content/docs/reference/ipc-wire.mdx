...
| Discovery | Side-effect-free `rpc.discover` before or after initialization (0.21+) |
| Cancellation | `rpc.cancel` notification naming the original request ID |
| Callbacks | `client.`-prefixed requests an endpoint sends to its client, only when advertised (0.21+) |
...
Discovery (0.21+) and initialization use the same absolute limit.
...
`rpc.discover` (0.21+), `rpc.initialize`, `rpc.cancel`, and `rpc.shutdown` are
...
state. It MAY be preceded only by `rpc.discover` (0.21+) requests. Clients that
...
`client_methods` (0.21+) lists the [callbacks](#callbacks) this client can
...
```json
{
  ...
  "result": {
    "protocol": "secretspec.resolver",
    "version": 1,
    "server": { "name": "secretspec-resolver", "version": "0.21.0" },
    "methods": ["resolver.get", "resolver.release"],
    "capabilities": {},
    "limits": {
      "max_frame_bytes": 1048576,
...
Server-initiated requests are available starting with SecretSpec 0.21.
...
Version 1 defines two callbacks (both available in SecretSpec 0.21+):
...
in-flight-limit checks. Every accepted application or `rpc.discover` (0.21+)
...
for `unavailable`. Starting in 0.21, an `interaction_required` error may instead
...
For optional connected Rust resolver sessions (0.21+), shutdown ends only the
