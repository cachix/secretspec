...
whitespace, and final newlines. Only zero-byte output is rejected. Use
`printf` or trim inside your command if you need a value without a final
newline. Binary output needs a byte-capable provider or manifest `encoding`.
