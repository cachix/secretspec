...
- **`ext-ffi`** dlopens the `libsecretspec` shared library at runtime. Nothing
...
The FFI backend dlopens the `libsecretspec` library at runtime. Enable the
...
That downloads the right `libsecretspec` library from the matching GitHub
...
Use `withInlineSpec($spec, $baseDir)` to resolve a strict inline-spec v2 PHP
array (SecretSpec 0.21+). The embedded extension or FFI fallback uses the
versioned native call; an older cdylib raises a capability error instead of
...
the SDK dlopens the `libsecretspec` library via `ext-ffi`, looking first at
