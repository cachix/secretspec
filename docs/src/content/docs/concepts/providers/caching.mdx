...
and v3 text entries remain readable during migration. SecretSpec 0.20 and
...
reference, or `max_age` invalidates it. In SecretSpec 0.21+, `value_base64` is
...
SecretSpec always writes v4 and continues reading v2/v3 text values as UTF-8
bytes.
