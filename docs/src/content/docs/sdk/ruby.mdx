...
Use `.with_inline_spec(spec, base_dir)` to resolve a strict inline-spec v2 hash
(SecretSpec 0.21+) at its logical provider-path base directory. The extension
links the separate native call symbol, so an older archive cannot fall back to
a manifest search.
