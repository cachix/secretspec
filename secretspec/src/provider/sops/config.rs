...
fn infer_format_from_pattern(pattern: &str) -> SopsFormat {
...
impl TryFrom<&ProviderUrl> for SopsConfig {
    ...
    fn try_from(url: &ProviderUrl) -> std::result::Result<Self, Self::Error> {
        ...
        let format = if let Some(fmt) = explicit_format {
            ...
            infer_format_from_pattern(&pattern)
        } else {
            match target_path.extension().and_then(|e| e.to_str()) {
                Some(ext) => {
                    SopsFormat::from_str(ext).expect("SOPS provider failed to infer format")
                }
                None => SopsFormat::Yaml,
            }
        };

        let mode = if pattern.is_empty() {
            Some(SopsMode::SingleFile(PathBuf::from(raw_path)))
        } else {
            if let Ok(path_pattern) = SopsPathPattern::try_from(pattern) {
                Some(SopsMode::Directory {
                    path: dir_path,
        ...
        if let Some(sops_mode) = mode {
            ...
            Err(SecretSpecError::ProviderOperationFailed(format!(
                "Failed to instantiate path patterning for SOPS provider"
            )))
        }
    }
}
