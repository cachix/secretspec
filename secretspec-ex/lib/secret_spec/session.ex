defmodule SecretSpec.Session do
  ...
  def handle_info({:prompt_result, id, answer}, state) do
    ...
    send_message(state, %{"jsonrpc" => "2.0", "id" => id, "result" => params})
    {:noreply, state}
  end
  ...
  defp handle_message(%{"method" => "client.prompt", "id" => id, "params" => params}, state)
       when is_function(state.prompt, 1) do
    parent = self()
  ...
  defp decode_result(%{"released" => true}), do: :ok
