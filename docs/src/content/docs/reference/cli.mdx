...
provider with no mappings (0.21+), starts the endpoint, prompts for the
...
mappings may still request credentials dynamically (0.21+).
...
Use `--config project` for the `secretspec.toml` editor schema or `--config global`
...
```bash
$ secretspec schema --config project
...
$ secretspec set <NAME> --from-file <FILE|-> # 0.21+
```
...
- `--from-file <FILE>` - Read the exact value bytes from `FILE`; use `-` for
  stdin (0.21+). This conflicts with the positional `VALUE`.

**Example:**
```bash
$ secretspec set API_KEY sk-1234567890 --profile production --provider sops://secrets.enc.yaml

$ secretspec set TLS_KEYSTORE --from-file client.p12 # 0.21+

$ generate-key | secretspec set TLS_KEYSTORE --from-file - # 0.21+
# SecretSpec 0.19+:
...
`echo value | secretspec set NAME` stores `value`; use `--from-file -` to keep
every byte. A binary declaration normally uses `as_path = true`; add a
manifest `encoding` when its provider accepts only text.

In SecretSpec 0.21+, an external IPC provider can request a missing required
password or other authentication credential during `set`. SecretSpec prompts
...
`serve` is available starting with SecretSpec 0.21.
...
Before initialization, inspection tooling may call `rpc.discover` (0.21+) to
...
**Options:**
- `--read-only` - Resolve only, never write to a provider (SecretSpec 0.21+)
...
The session also answers `resolver.set` and `resolver.delete` (0.21+), which
