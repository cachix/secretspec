...
The Claude Code credential integration is available in SecretSpec 0.21+. It
...
corresponding option explicitly when it should be pinned. `login`, `logout`,
and the helper also ignore an exported `SECRETSPEC_PROVIDER`, so they always
use the same store: the provider pinned by `configure`, or the SecretSpec user
default when none is pinned.
...
ordinary `secretspec get` command:
