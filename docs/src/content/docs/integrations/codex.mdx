...
The Codex API-key integration is available in SecretSpec 0.21+. It configures
...
- SecretSpec 0.21 or newer, with `secretspec` on `PATH`

## Quick start

<VersionCompatibility version="0.21" />

These commands are available in SecretSpec 0.21+.
...
User-level Codex API-key configuration is available in SecretSpec 0.21+.
...
Gateway configuration is available in SecretSpec 0.21+.
...
Custom Codex credential configuration is available in SecretSpec 0.21+.
...
These removal commands are available in SecretSpec 0.21+.
...
SecretSpec 0.21+ management commands are optional when a declarative Codex
...
can start outside the manifest's directory. In SecretSpec 0.21+, add `--caller
