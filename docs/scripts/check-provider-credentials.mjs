...
const rustSources = collectRustSources(path.join(repositoryDirectory, 'secretspec/src/provider'));
...
for (const entry of catalog) {
	...
	if (fs.existsSync(markdownPath)) {
		throw new Error(`${entry.provider}: rename the credential-aware provider page from .md to .mdx`);
	}
...
console.log(`Provider credential documentation is synchronized (${catalog.length} providers, ${credentialCount} credentials).`);
