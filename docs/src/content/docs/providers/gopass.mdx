...
SecretSpec writes lossless entries using `gopass cat` so whitespace and line
endings survive unchanged. Read these entries with `gopass cat`, which decodes
gopass's binary-entry format. Existing text entries retain password-only reads
...
Entries written by SecretSpec 0.20 or earlier, or created with `gopass insert`,
are text entries. SecretSpec reads only their first line and removes surrounding
whitespace, so a multiline value stored that way comes back truncated with no
error. Writing the secret again with `secretspec set` migrates the entry to the
lossless format, which preserves every byte.
