...
Add this comment at the top of `secretspec.toml` to enable completion, hover
...
For the user configuration, typically `~/.config/secretspec/config.toml`, use:

```toml
#:schema https://secretspec.dev/schema/config.schema.json
```

These are [Taplo schema directives](https://taplo.tamasfe.dev/configuration/directives.html#the-schema-directive),
so they remain ordinary comments to SecretSpec. The schemas describe the current
...
fields; SecretSpec still checks inheritance, references, provider support, and
other semantic constraints when loading the configuration.

You can also download the [project schema](/schema/secretspec.schema.json) or
...
With SecretSpec 0.21+, generate the schema from your installed CLI to match its
configuration types:
...
Then use `#:schema ./secretspec.schema.json` in a project manifest, or point the
...
composed secret materializes the final combined value. In SecretSpec 0.21+,
...
For an external provider (0.21+), `credentials` is optional and does not need
...
In SecretSpec 0.21+, the file always contains the exact logical bytes. When
...
the selected logical value. In 0.21+, `get` and Rust's `resolve_bytes()` and
...
is independent of the cache envelope and of `as_path`: in 0.21+, decoded bytes
...
In SecretSpec 0.21+, the stored representation must be ASCII and contain only
...
`Secrets::set` and `secretspec set --from-file` (0.21+) provide logical bytes;
...
arbitrary stdout bytes in 0.21+. SecretSpec encodes logical bytes
before writing to a provider. The `secretspec import` command copies the stored
...
| [file (0.19+)](/providers/file/#use-existing-files) | Relative file path beneath the configured root | Rejected | Reads the complete file as arbitrary bytes (0.21+) | ✅ |
...
| [systemd credentials (0.17+)](/providers/systemd-credential/#use-an-existing-credential-name) | Credential filename | Rejected | Reads arbitrary credential bytes (0.21+) | — (read-only) |
...
| [AWS Secrets Manager](/providers/awssm/#use-existing-secrets) | Secret name or ARN | JSON key (UTF-8 only) | Whole `SecretString` or `SecretBinary` value (0.21+) | — (read-only) |
...
| `command` | stdout of command; exact bytes in 0.21+ | `command` (string, required) |
