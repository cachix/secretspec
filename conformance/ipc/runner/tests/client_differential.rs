...
impl CClient {
    ...
    fn execute(&self, action: &Action) -> Result<Outcome, String> {
        let (params, deadline, cancel) = match action {
            Action::Echo { token } => (
                json!({
                    "mode": "echo",
                    "token": token
    ...
    fn close(mut self) -> Result<(), String> {
        ...
        let status = unsafe {
            secretspec_ipc_client_close(self.0, deadline_after(Duration::from_secs(2)), &mut error)
        };
...
fn copy_buffer(buffer: Buffer) -> Result<Vec<u8>, String> {
    ...
    // `size` bytes until `secretspec_ipc_buffer_free`.
