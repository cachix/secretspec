...
| Elixir (0.21+) | `secretspec` on Hex.pm, using `secretspec serve` | `elixir-package.yml` |

<VersionCompatibility version="0.21" />

The Elixir SDK is available from SecretSpec 0.21 and uses the shared resolver IPC protocol.
