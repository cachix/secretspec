...
mod tests {
    ...
    fn rejection_reports_the_layer_that_refused_the_frame() {
        ...
        for payload in [
            ...
            br#"{"jsonrpc":"2.0","id":0,"method":"x","deadline_unix_ms":1,"params":{}}"#.as_slice(),
            br#"{"jsonrpc":"2.0","id":1,"method":"","deadline_unix_ms":1,"params":{}}"#.as_slice(),
            br#"{"jsonrpc":"2.0","id":1,"method":"x","deadline_unix_ms":1,"params":[]}"#.as_slice(),
            br#"{"jsonrpc":"2.0"}"#.as_slice(),
        ] {
            let (_, kind) = Envelope::parse_classified(payload).unwrap_err();
    ...
    fn rejects_duplicate_keys_and_batches() {
        assert!(
            Envelope::parse(
                br#"{"jsonrpc":"2.0","jsonrpc":"2.0","id":1,"method":"x","deadline_unix_ms":1,"params":{}}"#
            )
            .is_err()
        );
    ...
    fn request_ids_are_bounded_integers() {
        for id in ["0", "-1", "1.5", "\"1\"", "9007199254740992"] {
            let input = format!(
                r#"{{"jsonrpc":"2.0","id":{id},"method":"x","deadline_unix_ms":1,"params":{{}}}}"#
            );
    ...
    fn rejects_unknown_top_level_members() {
        assert!(
            Envelope::parse(
                br#"{"jsonrpc":"2.0","id":1,"method":"x","deadline_unix_ms":1,"params":{},"extra":true}"#
            )
                .is_err()
        );
    }
    ...
    fn request_deadline_is_required_and_round_trips() {
        ...
        let parsed = Envelope::parse(
            br#"{"jsonrpc":"2.0","id":1,"method":"x","deadline_unix_ms":42,"params":{}}"#,
        )
        .unwrap();
