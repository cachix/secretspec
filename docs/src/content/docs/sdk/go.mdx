...
Inline specification v2 uses `project`, `profiles`, and each profile's
`secrets` object; optional `providers`, `scopes`, project `defaults` (0.21+),
profile `defaults`, and the normal secret declaration fields are also
supported. A 0.21+ library still accepts the v1 documents sent by 0.20 SDKs. Unknown declaration fields
