	...
	for (const entry of catalog) {
		...
		for (const credential of entry.credentials) {
			if (!CREDENTIAL_NAME.test(credential.name ?? '')) {
				errors.push(`${entry.provider}: invalid credential name ${JSON.stringify(credential.name)}`);
			}
			...
			for (const environment of credential.environmentFallbacks) {
				if (!ENVIRONMENT_NAME.test(environment)) {
					errors.push(`${entry.provider}.${credential.name}: invalid environment fallback ${JSON.stringify(environment)}`);
				}
				if (environments.has(environment)) {
					errors.push(`${entry.provider}.${credential.name}: duplicate environment fallback ${environment}`);
				}
...
function resolveCredentialToken(token, localConstants, globalConstants) {
	...
	if (values.size > 1) {
		throw new Error(`credential_names constant ${token} has conflicting values: ${[...values].join(', ')}`);
	}
	return [...values][0];
}
...
export function compareCatalogToRust(catalogByProvider, registeredProviders) {
	...
	for (const [provider, names] of registeredProviders) {
		...
		if (JSON.stringify(documented) !== JSON.stringify(names)) {
			errors.push(`${provider}: Rust credentials [${names.join(', ')}] do not match catalog credentials [${documented.join(', ')}]`);
		}
	}
	...
	for (const entry of catalog) {
		...
		for (const filename of entry.implementationSources) {
			try {
				...
				errors.push(`${entry.provider}: cannot read implementation source ${filename}: ${error.message}`);
			}
		}
		for (const credential of entry.credentials) {
			for (const environment of credential.environmentFallbacks) {
				if (!implementation.includes(`"${environment}"`)) {
					errors.push(`${entry.provider}.${credential.name}: environment fallback ${environment} is absent from its implementation sources`);
				}
			}
			if (
				...
				errors.push(`${entry.provider}.${credential.name}: ordered environment fallback sequence is absent from its implementation sources`);
			}
		}
	}
	fail(errors);
...
export function validateProviderPage(provider, content) {
	const errors = [];
	if (!/^import ProviderCredentials from ['"][^'"]*ProviderCredentials\.astro['"];?$/m.test(content)) {
	...
	if (headings.length !== 1) {
		errors.push(`${provider}: provider page must have exactly one \"## Provider credentials\" heading`);
	}
	const component = new RegExp(`<ProviderCredentials\\s+provider=["']${escapeRegExp(provider)}["']\\s*/>`);
	if (!component.test(content)) {
		errors.push(`${provider}: provider page does not render its ProviderCredentials entry`);
	}
	if (headings.length === 1) {
		const section = content.slice(headings[0].index, content.indexOf('\n## ', headings[0].index + 3) === -1 ? undefined : content.indexOf('\n## ', headings[0].index + 3));
		if (!component.test(section)) {
			errors.push(`${provider}: ProviderCredentials component must be inside its credential section`);
		}
	}
	fail(errors);
}
...
export function validateComponentBacklink(content) {
	if (!content.includes('/reference/provider-credentials/')) {
		throw new Error('ProviderCredentials component must backlink to the provider credential reference');
	}
}
