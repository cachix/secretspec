...
const nodeClient = (script: string, options: Omit<ConstructorParameters<typeof MonosecretClient>[0], 'executable' | 'executableArgs'> = {}) =>
...
describe('MonosecretClient.loadEnvironment', () => {
  ...
  it('parses environment lines and preserves equals signs in values', () => {
    expect(parseEnvironment('DATABASE_URL=postgres://localhost/app\nTOKEN=value=with=equals\nIGNORED_LINE\n')).toEqual({
      DATABASE_URL: 'postgres://localhost/app',
      TOKEN: 'value=with=equals',
    });
  });
