...
use super::{
    CredentialSource, CredentialSourceTable, ProviderAlias, ProviderAliasTable, RequireReason,
};
...
impl JsonSchema for ProviderAlias {
    ...
    fn json_schema(generator: &mut SchemaGenerator) -> Schema {
        ...
        table.insert("oneOf".into(), serde_json::json!([
            {
                "required": ["uri"],
...
pub(crate) fn generate(global: bool) -> Result<String, serde_json::Error> {
    ...
    let (mut schema, name, title) = if global {
        (
            ...
            "config",
            "SecretSpec user configuration",
        )
    } else {
        (
            generator.into_root_schema_for::<super::Config>(),
            "secretspec",
            "SecretSpec project configuration",
        )
    };
    schema.insert(
        "$id".into(),
        format!("https://secretspec.dev/schema/{name}.schema.json").into(),
    );
    schema.insert("title".into(), title.into());
    schema.insert("$comment".into(), "Generated by secretspec schema --config. Do not edit by hand. Runtime validation also checks cross-field and provider-specific constraints.".into());
