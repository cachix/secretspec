...
Inline values are written as exact bytes, including non-UTF-8 data and NULs,
without adding a newline. An `as_path` value still prints its materialized
filename followed by a newline.
</VersionCompatibility>
